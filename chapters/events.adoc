    ...
    ordering_key_fields:
      ...
      items:
        ...
        description: |
          ...
          case of a Lifecycle-Event) objects. It must point to a field of type
    ...
    ordering_instance_ids:
      ...
      items:
        ...
        description: |
          ...
          data (in case of a Lifecycle-Event) objects, and it must be present in the
          schema.
...
* Lifecycle-Event: a category to inform about data entity changes 
...
(e.g. the operation for Lifecycle-Events).
...
=== The Lifecycle-Event category

The _Lifecycle-Event Category_ structure is shown below as an OpenAPI
Schema Object:
...
LifecycleEvent:
  description: |
    ...
    LifecycleEvent's definition and the custom schema definition.
  required:
    - metadata
    - data_op
    - data_type
...
The Lifecycle-Event Category is structurally different to the General
...
The following guidelines specifically apply to Lifecycle-Events:
...
The General and Lifecycle-Event categories share a common structure
...
[#202]
== {MUST} use Lifecycle-Events to signal mutations

You *must* use Lifecycle-Events to signal changes of stored entity instances
...
[#242]
== {MUST} provide explicit event ordering for Lifecycle-Events

While the order information is recommended for business events, it *must* be 
provided for Lifecycle-Events. The ordering information defines the (create, 
...
[#204]
== {SHOULD} use the hash partition strategy for Lifecycle-Events
...
There may be exceptional cases where Lifecycle-Events could have their
...
[#205]
== {SHOULD} ensure that Lifecycle-Events match the APIs resources

A Lifecycle-Event's representation of an entity should correspond to
...
There are cases where it could make sense to define Lifecycle-Events
...
As common example similar to Lifecycle-Events, idempotent out-of-order
...
by a Lifecycle-Event and a RESTful API.
